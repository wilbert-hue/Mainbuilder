...
function findTopPerformer(records: DataRecord[], filters: FilterState, currency: 'USD' | 'INR' = 'USD'): Insight | null {
  ...
  records.forEach(record => {
    ...
    // Use MAX instead of SUM to match chart display values
  ...
  // Note: Data is already stored in millions (USD Mn) or appropriate units
  let valueDisplay = ''
  if (filters.dataType === 'value') {
    if (currency === 'INR') {
      // For INR, convert from USD millions to INR
      // Assume data is in USD millions, convert to INR (approx 83 INR per USD)
      const inrValue = topValue * 83 // Convert USD Mn to INR Mn
      if (inrValue >= 100) {
        valueDisplay = `₹${inrValue.toFixed(2)} Cr` // Already in millions, display as Cr
      } else if (inrValue >= 1) {
        valueDisplay = `₹${(inrValue * 100).toFixed(2)} L` // Convert Mn to Lakhs
      } else {
...
function findGrowthLeader(records: DataRecord[], filters: FilterState): Insight | null {
  ...
  // Calculate growth for each group using MAX values (to match chart display)
  grouped.forEach((groupRecords, key) => {
    ...
    groupRecords.forEach(record => {
      const recordStart = record.time_series[startYear] || 0
      const recordEnd = record.time_series[endYear] || 0
      if (recordStart > startValue) startValue = recordStart
      if (recordEnd > endValue) endValue = recordEnd
    })
