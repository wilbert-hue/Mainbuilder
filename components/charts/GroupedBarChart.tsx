...
export function GroupedBarChart({ title, height = 400 }: GroupedBarChartProps) {
  ...
  const chartData = useMemo(() => {
    ...
    console.log('📊 Chart Data Debug:', {
      ...
      // Show unique geographies in dataset
      uniqueGeographies: [...new Set(dataset.map(r => r.geography))].slice(0, 10),
      // Show unique segment types
      uniqueSegmentTypes: [...new Set(dataset.map(r => r.segment_type))].slice(0, 5)
    })
    ...
    console.log('📊 After filtering:', {
      ...
      // Show which geographies passed the filter
      filteredGeographies: [...new Set(filtered.map(r => r.geography))],
      // Show aggregation levels in filtered data
      filteredLevels: [...new Set(filtered.map(r => r.aggregation_level))]
    })
    ...
    // In geography-mode, we always show total per geography (no stacking)
    // Stacking only applies in segment-mode when multiple geographies are selected
    ...
    if (isStacked) {
      ...
      if (filters.aggregationLevel === 1) {
        ...
        // Geography Mode: show geographies as bars
        // Segment Mode: show segments as bars
  ...
  return (
    <div className="w-full min-w-0">
      {title && (
        <h3 className="text-lg font-semibold mb-4 text-black truncate" title={title}>{title}</h3>
      )}

      <ResponsiveContainer width="100%" height={height}>
        <BarChart data={chartData.data} margin={{ top: 5, right: 30, left: 60, bottom: 5 }}>
